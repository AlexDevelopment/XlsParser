...
public class LongTypeFieldWriter <T> implements TypeFieldWriter {

    private static LongTypeFieldWriter typeFieldWriter;
    private TypeAdapter typeAdapter = null;

    private LongTypeFieldWriter() {
    }

    static <T> LongTypeFieldWriter<T> getInstance() {
        if (typeFieldWriter == null) {
            typeFieldWriter = new LongTypeFieldWriter<T>();
        }
    ...
    private <T> void writeStringToField(Field field, Cell cell, T instant) throws IllegalAccessException {
        if (typeAdapter != null) {
            field.set(instant, typeAdapter.write(Long.parseLong(cell.getStringCellValue())));
        } else {
            field.set(instant, Long.parseLong(cell.getStringCellValue()));
        }
    }
    ...
    private <T> void writeBooleanToField(Field field, T instant, Cell cell) throws IllegalAccessException {
        if (typeAdapter != null) {
            field.set(instant, typeAdapter.write(getLongByBoolean(cell.getBooleanCellValue())));
        } else {
            field.set(instant, getLongByBoolean(cell.getBooleanCellValue()));
        }
    }

    private long getLongByBoolean(Boolean value){
        if(value){
            return 0;
        } else {
            return 1;
    ...
    private <T> void writeNumericToField(Field field, T instant, Cell cell) throws IllegalAccessException {
        if (typeAdapter != null) {
            field.set(instant, typeAdapter.write((long)cell.getNumericCellValue()));
        } else {
            field.set(instant, (long)cell.getNumericCellValue());
        }
    }
}
